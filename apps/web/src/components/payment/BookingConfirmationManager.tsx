...
export default function BookingConfirmationManager({
  ...
  const getOrCreateUser = async (): Promise<number> => {
    ...
    try {
      ...
      const result = await apiCall(`${API_BASE_URL}/api/auth/register`, 'POST', userData);
      ...
      if (error.message.includes('existe déjà') || error.message.includes('already exists')) {
        ...
        try {
          const loginResult = await apiCall(`${API_BASE_URL}/api/auth/login`, 'POST', {
            email: bookingData.guestInfo.email,
...
const generateTemporaryPassword = (): string => {
  ...
  const createPendingReservation = async (userId: number) => {
    ...
    const result = await apiCall(`${API_BASE_URL}/api/reservations`, 'POST', reservationData, token);
  ...
  try {
    ...
    const paymentResult = await apiCall(`${API_BASE_URL}/api/payments`, 'POST', paymentPayload, token);
    ...
    await apiCall(`${API_BASE_URL}/api/reservations/${currentReservationId}`, 'PATCH', updateReservationPayload, token);
